...
try:  # use cmocean colormaps if installed else matplotlib #not working withe regstration workaround here
    ...
    # Register cmocean colormaps manually
    cmocean_colormaps = {name: getattr(cmocean.cm, name) for name in cmocean.cm.cmapnames}
    for name, cmap in cmocean_colormaps.items():
        try:
            plt.register_cmap(name=name, cmap=cmap)
        except ValueError:
            pass  # Skip colormaps that already exist in Matplotlib
    use_cmocean = True
...
class Window(tk.Frame):
    ...
    def find_parent_tri(self, tris, xun, yun, xq, yq, dThresh=1000):
        ...
        for ip in range(len(p)):
            ...
            parenti = (subtris[(dx1[subind] * SideX[subind, 0] + dy1[subind] * SideY[subind, 0] <= 0)
                               & ((p[ip, 0] - trisX[subind, 1]) * SideX[subind, 1] + (p[ip, 1] - trisY[subind, 1]) *
                                  SideY[subind, 1] <= 0)
                               & ((p[ip, 0] - trisX[subind, 2]) * SideX[subind, 2] + (p[ip, 1] - trisY[subind, 2]) *
                                  SideY[subind, 2] <= 0)][:])
            if len(parenti):
                parent[ip] = parenti
        ...
        # barycentric weights
        divisor = (yabc[:, 1] - yabc[:, 2]) * (xabc[:, 0] - xabc[:, 2]) + (xabc[:, 2] - xabc[:, 1]) * (
                    yabc[:, 0] - yabc[:, 2])
    ...
    def vert_int(self, avg=False):
        zcor = self.ncs[self.vardict[self.zcorname]][self.zcorname][self.total_time_index, :,
               :].values  # self.ti_tk.get() #self.ncv['zcor']
    ...
    def schism_updateAtelems(self):
        ...
        #case analysis data spatial field    
        elif self.shape[0] == 1: # 1 time steo
            self.ncvar = self.ncs[self.vardict[self.varname]][self.varname]  #
            self.ncvar = self.ncvar[0, :]
        ...
        if (self.shape == (self.nt, self.nnodes, self.nz)) | (self.shape == (self.nts[0], self.nnodes, self.nz)):
            ...
            if self.CheckFixZ.get() == 1:  # z interpolation
                ...
                self.nodevalues = weights[0, :] * self.ncvar[:, :].values[self.nodeinds, ibelow] + weights[1, :] * \
                                  self.ncvar[:, :].values[self.nodeinds, iabove]
        ...
        elif (self.shape == (self.nnodes,)) or (self.shape == (1,self.nnodes)):   #added after or for testing
        ...
        # add at element variables variables
            ...
            if self.CheckDiff.get() == 1:  # plot diffrence between absolute values
                uabs1 = np.sqrt((self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][:,
                                 self.total_time_index, :] ** 2).sum(axis=0))
                uabs0 = np.sqrt(
                    (self.nclist[0][self.vardict[self.varname]][self.varname][:, self.total_time_index, :] ** 2).sum(
                        axis=0))
                self.nodevalues = (uabs1 - uabs0).values
            else:
                if self.CheckTavg.get():
                    self.nodevalues = np.sqrt(
                        (self.ncs[self.vardict[self.varname]][self.varname] ** 2).sum(axis=0)).values
                else:
                    self.nodevalues = np.sqrt(u * u + v * v)
            ...
            # v=np.ma.masked_array(v,mask=np.isnan(v))
                ...
                self.shape == (2, self.nts[0], self.nnodes, self.nz)):
            if self.CheckFixZ.get() == 1:  # vertical interpol (new code)
                ...
                u = weights[0, :] * self.ncvar[0, :, :].values[self.nodeinds, ibelow] + weights[1, :] * \
                    self.ncvar[0, :, :].values[self.nodeinds, iabove]
                v = weights[0, :] * self.ncvar[1, :, :].values[self.nodeinds, ibelow] + weights[1, :] * \
                    self.ncvar[1, :, :].values[self.nodeinds, iabove]

                if self.CheckDiff.get() == 1:  # plot diffrence between absolute values
                    u0 = weights[0, :] * \
                         self.nclist[0][self.vardict[self.varname]][self.varname][0, self.total_time_index, :,
                         :].values[self.nodeinds, ibelow] + weights[1, :] * \
                         self.nclist[0][self.vardict[self.varname]][self.varname][0, self.total_time_index, :,
                         :].values[self.nodeinds, iabove]
                    v0 = weights[0, :] * \
                         self.nclist[0][self.vardict[self.varname]][self.varname][1, self.total_time_index, :,
                         :].values[self.nodeinds, ibelow] + weights[1, :] * \
                         self.nclist[0][self.vardict[self.varname]][self.varname][1, self.total_time_index, :,
                         :].values[self.nodeinds, iabove]
                    u1 = weights[0, :] * self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][0,
                                         self.total_time_index, :, :].values[self.nodeinds, ibelow] + weights[1, :] * \
                         self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][0,
                         self.total_time_index, :, :].values[self.nodeinds, iabove]
                    v1 = weights[0, :] * self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][1,
                                         self.total_time_index, :, :].values[self.nodeinds, ibelow] + weights[1, :] * \
                         self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][1,
                         self.total_time_index, :, :].values[self.nodeinds, iabove]
                ...
                if self.oldio:
                    ...
                    if self.CheckDiff.get() == 1:  # plot diffrence between absolute values
                        uabs1 = np.sqrt(
                            self.nclist[self.active_setup]['schout']['hvel'][self.total_time_index, :, self.lvl, :].sum(
                                axis=0))
                        uabs0 = np.sqrt(
                            self.nclist[0]['schout']['hvel'][self.total_time_index, :, self.lvl, :].sum(axis=0))
                    ...
                    if self.CheckDiff.get() == 1:  # plot diffrence between absolute values
                        uabs1 = np.sqrt((self.nclist[self.active_setup][self.vardict[self.varname]][self.varname][:,
                                         self.total_time_index, :, self.lvl] ** 2).sum(axis=0))
                        uabs0 = np.sqrt((self.nclist[0][self.vardict[self.varname]][self.varname][:,
                                         self.total_time_index, :, self.lvl] ** 2).sum(axis=0))

            if self.CheckDiff.get() == 0:  # plot diffrence between absolute values

                if self.CheckTavg.get():
                    print('using temporal avegarge of nodes {:s}-{:s} - not suitable if large variations in z'.format(
                        self.exfrom.get(), self.exto.get()))
                    self.nodevalues = np.sqrt(
                        (self.ncs[self.vardict[self.varname]][self.varname][:, :, :, self.lvl] ** 2).sum(axis=0)).mean(
                        axis=0).values
                else:
                    self.nodevalues = np.sqrt(u * u + v * v)
                # self.nodevalues=np.sqrt(u*u+v*v)
            ...
            print(
                'variable shape missmatch - happened e.g. for unfinished runs with different outputwriting in variable files - assuming 3d file ')
            if self.CheckFixZ.get() == 0:
                self.nodevalues = self.ncvar[self.total_time_index, :, self.lvl].values
            else:  # z interpolation
                self.nodevalues = weights[0, :] * self.ncvar[self.total_time_index, :, :].values[
                    self.nodeinds, ibelow] + weights[1, :] * self.ncvar[self.total_time_index, :, :].values[
                                      self.nodeinds, iabove]
                # optimize indexing
                self.nodevalues = np.ma.masked_array(self.nodevalues, mask=np.isnan(self.nodevalues))
            title = self.varname
        ...
        if self.CheckEval.get() != 0:  # evaluate on displayed variable
            ...
            expr = expr[expr.index('=') + 1:].replace('x', 'self.nodevalues').replace('A', 'self.A').replace('dt',
                                                                                                             'self.dt')
        ...
        # setting colorbar
        if elem_data:
            elemvalues = np.ma.masked_array(self.nodevalues[self.origins],
        ...
        if self.quivVar.get():
            ...
            if self.normVar.get() == 1:
                ...
                self.quiver = plt.quiver(np.concatenate((self.plotx[qloc], (xref,))),
                                         np.concatenate((self.ploty[qloc], (yref,))),
                                         np.concatenate((u[qloc] / vabs, (1,))), np.concatenate((v[qloc] / vabs, (0,))),
                ...
                self.quiver = plt.quiver(np.concatenate((self.plotx[qloc], (xref,))),
                                         np.concatenate((self.ploty[qloc], (yref,))),
                                         np.concatenate((u[qloc], (vmax,))), np.concatenate((v[qloc], (0,))),
                                         scale=2 * vmax, scale_units='inches')
    ...
    def titlegen(self, lvl):
        ...
        if self.oldio:
            return prefix + self.varname + ' @ ' + str(self.ncs[self.filetag]['time'][self.total_time_index].values)[
                                                   :16] + ' level= ' + str(lvl)
        elif self.newio < 2:  # new io
            return prefix + self.varname + ' @ ' + str(self.reftime + dt.timedelta(
                seconds=int(self.ncs[self.filetag]['time'][self.total_time_index]))) + ' level= ' + str(lvl)
        else:
            return prefix + self.varname + ' @ ' + str(self.ncs[self.filetag]['time'][self.total_time_index].values)[
                                                   :19] + ' level= ' + str(lvl)
    ...
    def load_setup_data(self, ncdirsel=None):
        ...
        if self.oldio:
            ...
            ncs = {'schout': []}
            ncs['schout'] = xr.concat(
                [xr.open_dataset(self.combinedDir + 'schout_' + str(nr) + '.nc').chunk() for nr in self.stack_nrs],
                dim='time')
            ...
            exclude = ['time', 'SCHISM_hgrid', 'SCHISM_hgrid_face_nodes', 'SCHISM_hgrid_edge_nodes',
                       'SCHISM_hgrid_node_x',
                       ...
                       'sigma', 'dry_value_flag', 'coordinate_system_flag', 'minimum_depth', 'sigma_h_c',
                       'sigma_theta_b',
                       'sigma_theta_f', 'sigma_maxdepth', 'Cs', 'wetdry_node', 'wetdry_elem',
                       'wetdry_side']  # exclude for plot selection
            ...
            for vari in self.ncv:
                if vari not in exclude:
                    if self.ncv[vari].shape[-1] == 2:
                        ...
                        ncs[vari] = {vari: xr.concat([ncs['schout'][vari].sel(two=0), ncs['schout'][vari].sel(two=1)],
                                                     dim='ivs')}
                    else:
            ...
            varfiles = [file[file.rindex('/') + 1:file.rindex('_')] for file in
                        glob.glob(self.combinedDir + '*_' + str(self.stack_nrs[0]) + '.nc')]
            ncs = dict.fromkeys(varfiles)
            try:
                for var in varfiles:
                    ncs[var] = xr.concat(
                        [xr.open_dataset(self.combinedDir + var + '_' + str(nr) + '.nc').chunk() for nr in
                         self.stack_nrs], dim='time')
            except:
            ...
            exclude = ['time', 'SCHISM_hgrid', 'SCHISM_hgrid_face_nodes', 'SCHISM_hgrid_edge_nodes',
                       ...
                       'sigma', 'dry_value_flag', 'coordinate_system_flag', 'minimum_depth', 'sigma_h_c',
                       'sigma_theta_b',
                       'sigma_theta_f', 'sigma_maxdepth', 'Cs', 'dryFlagElement',
                       'dryFlagSide']  # exclude for plot selection
            vector_vars = []  # stack components for convenience
            self.vardict = {}  # variable to nc dict relations
            ...
            for vari_vec in vector_vars:
                ...
                ncs[vari_vec] = {
                    vari_vec: xr.concat([ncs[self.vardict[varX]][varX], ncs[self.vardict[varY]][varY]], dim='ivs')}
        ...
        if len(self.nclist) > 1:
            ...
            for key in self.nclist[0].keys():
                if key=='out2d':
                    ncdiff[key] = xr.Dataset({var: self.nclist[isetup][key][var] - self.nclist[0][key][var] for var in self.nclist[isetup][key].data_vars})
    ...
    def init_window(self, ncdirsel=None):
        ...
        self.minavgdist = np.min(np.sqrt(np.abs(np.diff(self.x[self.faces[:, [0, 1, 2, 0]]], axis=1)) ** 2 + np.abs(
            np.diff(self.y[self.faces[:, [0, 1, 2, 0]]], axis=1)) ** 2).mean(axis=1))
        ...
        if self.hasquads:  # build tri only grid for faster plotting
            self.quadpc = PolyCollection(self.mesh_quads, facecolors='none', edgecolors='r',
                                         linewidth=0.2)  # , **kwargs)
        ...
        w4 = tk.Spinbox(self, from_=0, to=levels[-1], width=5, validate='all',
                        textvariable=self.lvl_tk)  # try command and
        ...
        w = tk.Spinbox(self, values=self.stack_nrs, width=5, validate='all',
                       textvariable=self.stack)  # try command and set values in function with checking
        ...
        if self.use_cmocean:
            #maps = cmo.cmapnames
            maps = {name: getattr(cmocean.cm, name) for name in cmocean.cm.cmapnames}
    ...
    def variable_selection(self, event):  # combobox callback done differntly  #not used iguess
    ...
    def stream2image(self, *args):
        ...
        for nr, fig in enumerate(plt.get_fignums()):
            plt.figure(fig)
            plt.savefig(self.streamdir + '_{:d}_'.format(nr) + '{0:05d}'.format(
                self.total_time_index) + '_' + self.varname + '.png', dpi=300)
    ...
    def cmap_callback(self, *args):
        if self.use_cmocean:
            ...
            #from IPython import embed; embed()
            #cmap = eval('{:s}'.format(self.cmap.get()))
...
#
    ...
    def ask_coordinates(self, n=-1, coords=None, interp=False):
        ...
        nr = self.fignums
        self.plot_windows[self.fignums] = {'fh': fig2, 'nr': nr, 'anno': anno, 'type': self.timeseries,
                                           'coords': self.coords.copy(), 'extract': self.extract, 'xs': self.xs.copy()}
        self.plot_windows[self.fignums]['p0'] = self.pt0i
        self.plot_windows[self.fignums]['varname'] = self.varname
        self.plot_windows[self.fignums]['setup'] = self.active_setup
    ...
    def timeseries(self, coords=None):
        ...
        # elif self.shape==(self.nt,self.nnodes,2)
            ...
            if self.npt == 1:
                self.ts = self.ncs[self.vardict[self.varname]][self.varname][:, i0:i1, self.nn].values.reshape(2,
                                                                                                               i1 - i0,
                                                                                                               1)
            else:
            ...
            if self.npt == 1:
                self.ts = self.ncs[self.vardict[self.varname]][self.varname][:, i0:i1, self.nn,
                          self.lvl].values.reshape(2, i1 - i0, 1)
            else:
        ...
        if self.ivs == 1:
            if self.CheckEval.get() != 0:
                expr = self.evalex.get()
                expr = expr[expr.index('=') + 1:].replace('x', 'self.ts').replace('A', 'self.A').replace('dt',
                                                                                                         'self.dt')
                self.ts = eval(expr)
            ...
            plt.grid()
            self.lh = plt.legend(['P' + str(i + self.pt0i) for i in range(self.npt)], loc='upper center',
            ...
            for iplt in range(1, 3):
                ...
                plt.grid()
                if iplt == 1:
                    plt.legend(['P' + str(i + self.pt0i) for i in range(self.npt)], loc='upper center',
                               bbox_to_anchor=(0.5, 1.3), ncol=6)
    ...
    def profiles(self, coords=None):
        ...
        if self.ivs == 1:
            ...
            for iplt in range(1, 3):
                ...
                if iplt == 1:
                    if self.newio < 2:
                        plt.title(str(self.reftime + dt.timedelta(
                            seconds=int(self.ncs[self.filetag]['time'][self.total_time_index].values))))
                    else:
    ...
    def hovmoeller(self, coords=None):
        ...
        if self.shape[1:3] == (self.nnodes, self.nz):  # self.shape[:3]==(self.nt,self.nnodes,self.nz):
            ...
            self.ts = np.squeeze(self.ncs[self.vardict[self.varname]][self.varname][:, i0:i1, self.nn, :])
        elif self.shape[1:] == (self.nnodes,) or self.shape[1:] == (
        self.nnodes, 2):  # self.shape==(self.nt,self.nnodes) or self.shape==(self.nt,self.nnodes,2):
        ...
        if self.ivs == 1:
            ...
            for iplt in range(1, 3):
                plt.subplot(3, 1, iplt)
                plt.pcolor(np.tile(self.t, (self.nz, 1)).transpose(), self.zcor,
                           np.squeeze(self.ts[iplt - 1, :, :]))  # /86400#squeeze(self.ts[:,:,iplt-1]))#/86400
                plt.ylabel('depth')
                plt.title(self.varname)
                plt.colorbar()
            ...
            plt.subplot(3, 1, 3)
            plt.pcolor(np.tile(self.t, (self.nz, 1)).transpose(), self.zcor,
                       np.sum(np.sqrt(self.ts ** 2), axis=0))  # /86400
    ...
    def hovmoeller_horz(self, coords=None):
        ...
        if self.ivs == 1:
            ...
            for iplt in range(1, 3):
                plt.subplot(3, 1, iplt)
                plt.pcolormesh(np.tile(self.t, (self.nz, 1)).transpose(), self.xs,
                               np.squeeze(self.ts[iplt - 1, :, :]))  # /86400#squeeze(self.ts[:,:,iplt-1]))#/86400
                plt.ylabel('depth')
                plt.title(self.varname)
                plt.colorbar()
    ...
    def bptransect_callback(self, coords=None):
        filename = filedialog.askopenfilename(title='opeb build point file', initialdir=self.runDir,
                                              filetypes=(('bp files', '*.bp'), ('All files', '*.*')))
    ...
    def transect_callback(self, coords=None):
        ...
        ylim = ((np.nanmin(self.zi), np.round(np.nanmax(self.zi),1) ))  # nan
        plt.figure(self.activefig)
        plt.clf()
        if self.ivs == 1:  # scalar # interpolation along sigma layers
            ...
            if m_interp == 'dinv':
                ...
                for i in range(len(self.parents)):
                    ...
                    self.dataTrans[i, ibttm:] = scipy.interp(self.zi[i, ibttm:], depths[0, ibttm:],
                                                             lvldata[0, ibttm:]) * w[i, 0] + \
                                                scipy.interp(self.zi[i, ibttm:], depths[1, ibttm:],
                                                             lvldata[1, ibttm:]) * w[i, 1] + \
                                                scipy.interp(self.zi[i, ibttm:], depths[2, ibttm:],
                                                             lvldata[2, ibttm:]) * w[i, 2]
                ...
                self.dataTrans = np.ma.masked_array(self.dataTrans,
                                                    mask=mask[qloc])  # self.mask3d
            ...
            if self.proj_transects:
                ...
                if self.calc_transport:
                    ...
                    if not self.activefig in self.transect_transport.keys():
                        self.transect_transport[self.activefig] = {'t': self.dates, 'Q': np.zeros(self.nt),
                                                                   'Qpos': np.zeros(self.nt), 'Qneg': np.zeros(self.nt)}
            ...
            if self.newio < 2:
                plt.title(str(self.reftime + dt.timedelta(
                    seconds=int(self.ncs[self.filetag]['time'][self.total_time_index]))))
            else:
                plt.title(str(self.ncs[self.filetag]['time'][self.total_time_index].values))
            ...
            for iplt in range(0, 2):
                ...
                if self.quivVar.get():
                    ...
                    w = self.ncs[self.vardict[self.vertvelname]][self.vertvelname][self.total_time_index, :, :].values[
                        self.nn, :] * 100
                    print('!vertical componend is exaggerated by factor 100')
                    plt.quiver(self.xs[::dx, ::dy], self.zi[::dx, ::dy], self.dataTrans[0, :, :][::dx, ::dy],
                               w[::dx, ::dy], color='w')
    ...
    def savenodevalues(self, *args):
        outname = filedialog.asksaveasfilename(
            filetypes=(("text files ", "*.txt"), ("all files", "*.*"), ("nc files", "*.nc")))
        ...
        if ext != 'nc':
            ...
            varout = ncout.createVariable('SCHISM_hgrid_face_nodes', 'i',
    ...
    def comp_obs(self, *args):
        ...
        obsfile = filedialog.askopenfile(title='select observation file',
                                         filetypes=(('bp files', '*.nc'), ('All files', '*.*'))).name
        self.dates = np.asarray(
            [self.reftime + dt.timedelta(seconds=ti) for ti in self.ncs[self.filetag]['time'].values], np.datetime64)
        if self.newio < 2:
            self.dates = np.asarray(
                [self.reftime + dt.timedelta(seconds=ti) for ti in self.ncs[self.filetag]['time'].values],
                np.datetime64)
